"""Export explicit email v2 while retaining the immutable v1 schema archive."""
...
def main() -> None:
    ...
    frozen = json.loads((ROOT / "reference/email-v1-frozen.json").read_text())
    ...
    for name, model in MODELS:
        ...
        filename = f"{name}.v2.schema.json"
        for root in (ROOT / "reference/schemas", PACKAGED / "schemas"):
            write(root / filename, schema)
        catalog["contracts"].append(
            dict(name=name, version="2", filename=filename, schema_digest=canonical_digest(schema))
        )
    ...
    for root in (ROOT / "reference/examples", PACKAGED / "reference_artifacts"):
        write(root / "email-program-suite-v2.json", suite)
        write(root / "email-effect-family-v2.json", families)
    vectors = []
    for run in runs:
