"""Export explicit email v4 while retaining the immutable v1, v2 and v3 schemas."""
...
def main() -> None:
    ...
    for name, model in MODELS:
        ...
        filename = f"{name}.v4.schema.json"
        for root in (ROOT / "reference/schemas", PACKAGED / "schemas"):
            write(root / filename, schema)
        catalog["contracts"].append(
            dict(name=name, version="4", filename=filename, schema_digest=canonical_digest(schema))
        )
    ...
    for root in (ROOT / "reference/examples", PACKAGED / "reference_artifacts"):
        write(root / "email-program-suite-v4.json", suite)
        write(root / "email-effect-family-v4.json", families)
    vectors = []
    for run in runs:
