...
## Campaigns, newsletters and linear sequences

`EmailCampaignPlan` coordinates an objective, audience intent, campaign window,
CTAs, policy references and measurements. A newsletter is one periodic broadcast;
a sequence is an immutable ordered revision with entry, exit, conversion,
suppression, quiet-hours and migration policy references. These are design inputs,
not a workflow interpreter.

`plan_email_message` accepts an existing `NewsletterIssuePlan` as an optional
specialization input, verifies its publication and audience policies, and binds
the old issue revision into the new common message plan. Existing newsletter
contracts remain unchanged; migration requires explicit selection of the issue
subject/preheader and supplied source sections and email compliance declarations.

Metric acquisition occurs outside Creator. `assess_email_program` receives
receipt-linked aggregates and preserves partial coverage and attribution gaps.
