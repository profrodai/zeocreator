...
The digest is a proposed approval target, not proof of human approval. Draft, test,
broadcast, sequence activation, enrolment, pause, cancellation and retirement
require separate operation proposals. Runtime resolves the public Zeocore
`CapabilityId`, verifies the operation schema digest and supported semantics,
and authorizes each exact effect. See [email marketing](email-marketing.md).
