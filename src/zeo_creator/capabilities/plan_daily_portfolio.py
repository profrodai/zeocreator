...
class PlanDailyPortfolioRequest(CreatorModel):
...
class PlanDailyPortfolioResponse(CreatorModel):
    plan: DailyEditorialPlan


@capability(
    id="creator.plan_daily_portfolio@1.0.0",
    description="Create a publication-isolated, evidence-backed daily editorial portfolio.",
    ...
    projection_name="creator_plan_daily_portfolio",
)
def plan_daily_portfolio(
    request: PlanDailyPortfolioRequest,
    ctx: ToolContext,
) -> CapabilityResult[PlanDailyPortfolioResponse]:
    del ctx
    try:
        plan = plan_portfolio(
    ...
    return CapabilityResult.ok(
        data=PlanDailyPortfolioResponse(plan=plan),
